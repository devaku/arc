...
export default function CameraPage() {
	...
	const [gridColors, setGridColors] = useState<string[][]>(
		Array(3).fill(null).map(() => Array(3).fill('#FFFFFF'))
	);
	...
	function handleUnsaveClick() {
		...
		setIsSaved(false);
		setSavedColors(Array(3).fill(null).map(() => Array(3).fill('#FFFFFF')));
		startInterval();
	}
		...
		<div className="flex flex-col mt-5 px-4">
			...
			<div className="flex justify-center gap-2 mb-4">
				<button
					...
					className="cursor-pointer p-2 rounded bg-green-500 text-black font-semibold"
				>
					Start Scanning
				</button>
				<button
					onClick={handleStopClick}
					className="cursor-pointer p-2 rounded bg-red-500 text-white font-semibold"
				>
					Stop Scanning
				</button>
				{!isSaved ? (
					<button
						...
						className="cursor-pointer p-2 rounded bg-blue-500 text-white font-semibold"
					>
						Save Colors
					</button>
				) : (
					<button
						...
						className="cursor-pointer p-2 rounded bg-yellow-500 text-black font-semibold"
					>
						Unsave & Resume
					</button>
				)}
			...
			<div className="text-center mb-4">
				<h1 className="text-2xl font-bold">Rubik's Cube Scanner</h1>
			...
			<div className="flex flex-col lg:flex-row justify-center items-start gap-8">
				...
				<div className="flex flex-col items-center">
					...
					<div className="grid grid-cols-3 gap-2 p-4 bg-gray-800 rounded-lg">
						{(isSaved ? savedColors : gridColors).map((row, rowIndex) => (
